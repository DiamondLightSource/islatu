...
def test_process_xrr_01(process_xrr_path):
    ...
    """
    proc = subprocess.run([process_xrr_path], capture_output=True, text=True)

    error_type = proc.stderr.split("\n")[3].split(":")[0].strip()
    assert error_type == "FileNotFoundError"


def test_process_xrr_02(process_xrr_path, path_to_resources, tmp_path, old_dcd_data):
    """
    ...
    proc = subprocess.run(
        [process_xrr_path, "-d", path_to_resources, "-y", yaml_path, "-o", tmp_path],
        capture_output=True,
        text=True,
    )
    ...
    print(proc.stdout)
    print(proc.stderr)
    assert proc.stdout.split("\n")[204].strip().startswith("Reduced data stored at ")
...
def test_process_xrr_03(process_xrr_path, path_to_resources, tmp_path, old_dcd_data):
    """
