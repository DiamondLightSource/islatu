...
import os
import ast
from schema import Or, And, Use,Schema, SchemaError,Optional
...
def validate_instrument(inst):
    ...
    if inst not in valid_instruments:
        raise ValueError(f"instrument {inst} not in valid instrument list {valid_instruments}")
    return True
...
def validate_crop_method(method):
    ...
    if method not in valid_methods:
        raise ValueError(f"method {method} not in valid crop method list {valid_methods}")
    return True
...
def validate_new_axis(name):
    ...
    if name not in valid_new_axes:
        raise ValueError(f"axis name {name} not in valid new axis list {valid_new_axes}")
    return True
...
transmission_schema=Schema({
    "values": lambda t: isinstance(t, list) and len(t) == 2 and all(isinstance(x, float) for x in t),
})

setup_schema=Schema({
    "sample size":And( Use(lambda s: ast.literal_eval(s)),  # Convert string to tuple
    lambda t: isinstance(t, tuple) and len(t) == 2 and all(isinstance(x, float) for x in t)),
