...
class DataSource:
    ...
    def __init__(self, title, origin=Origin(), experiment=Experiment(), links=None):
        self.origin = origin
...
class ProcessArgs:
    ...
    def resolve_defaults(self):
        ...
        self.jobfile_template = Path(
            self.jobfile_template or self.version_path / "template.sh"
        )
    ...
    def parse_yaml(self):
        ...
        if os.path.exists(self.processing_path):
            files.extend(
                [self.processing_path + x for x in os.listdir(self.processing_path)]
            )
        ...
        if len(yaml_files) != 1:
            ...
            if len(yaml_files) > 1:
                generic_err_str += "Names of found files are: " + str(yaml_files) + "."
    ...
    def parse_scan_numbers(self):
        ...
        if self.scan_numbers is None:
            ...
            nexus_files = [x for x in os.listdir(self.data_path) if x.endswith(".nxs")]
    ...
    def create_jobfile(self):
        if self.jobfile_template is None:
            self.jobfile_template: str = Path(
                f"{self.version_path}/islatu/CLI/islatuscript_template.sh"
            )
    ...
    def run_cluster_job(self):
        ...
        if limit == 1:
            ...
            process = subprocess.Popen(
                ["tail", "-f", f"{Path.home()}/islatu//{endslurms[-1]}"],
    ...
    def parse_and_reduce(self):
        ...
        if not self.cluster:
            i07reduce_noload(
                ...
                q_subsample_dicts=self.limit_q,
            )
        else:
...
class Foreperson:
    ...
    def __init__(self, run_numbers, yaml_file, directory, title,reduction_parser=io.i07_nxs_parser):
        ...
        if path.isdir(directory_path):
            ...
            raise FileNotFoundError(
                "The experiment directory <" + directory_path + "> cannot be found."
            )
    ...
    def setup(self, recipe):
        ...
        if "setup" in keys:
            ...
            if "beam width" in recipe["setup"].keys():
                ...
                raise ValueError(f"No beam width given in setup of {self.yaml_file}")
            if "theta axis" in recipe["setup"].keys():
                self.data_source.experiment.measurement.theta_axis_name = recipe[
                    "setup"
                ]["theta axis"]
            if "q axis" in recipe["setup"].keys():
                self.data_source.experiment.measurement.q_axis_name = recipe["setup"][
                    "q axis"
                ]
            if "transpose" in recipe["setup"].keys():
                self.data_source.experiment.measurement.transpose = recipe["setup"][
                    "transpose"
                ]
            ...
            if "pixel max" in recipe["setup"].keys():
                self.data_source.experiment.measurement.pixel_max = recipe["setup"][
                    "pixel max"
                ]
            if "hot pixel max" in recipe["setup"].keys():
                self.data_source.experiment.measurement.hot_pixel_max = recipe["setup"][
                    "hot pixel max"
                ]
        else:
        ...
        if "rebin" in keys:
            if "n qvectors" in recipe["rebin"].keys():
                ...
                "min" in recipe["rebin"].keys()
                and "max" in recipe["rebin"].keys()
                and "step" in recipe["rebin"].keys()
            ):
                ...
                raise ValueError(
                    "Please define parameters of " + f"rebin in {self.yaml_file}."
                )
        else:
...
def i07reduce(
    ...
    if hasattr(the_boss.reduction, "adjustments"):
        refl = Profile.fromfilenames(
            files_to_reduce,
            the_boss.reduction.parser,
            adjustments=the_boss.reduction.adjustments,
        )
    else:
    ...
    if (
        the_boss.reduction.crop_function is not cropping.crop_to_region
        and the_boss.reduction.crop_function is not None
    ):
    ...
    if (
        the_boss.reduction.crop_function is cropping.crop_to_region
        and the_boss.reduction.crop_kwargs is None
    ):
        try:
            roi = refl.scans[0].metadata.signal_regions[0]
        except IndexError:
            debug.log(
                "No valid signal region found, regions will need to be defined in yaml settings file"
            )
            return
    ...
    if the_boss.reduction.bkg_function is background.roi_subtraction:
        ...
        if the_boss.reduction.bkg_kwargs is None:
            ...
            the_boss.reduction.bkg_kwargs = {
                "list_of_regions": Region.from_dict(
                    region_dict=the_boss.reduction.bkg_kwargs
                )
            }

        refl.bkg_sub(the_boss.reduction.bkg_function, **the_boss.reduction.bkg_kwargs)
    ...
    if q_subsample_dicts is not None:
        log_processing_stage("Bounding q-vectors.")
    ...
    if the_boss.data.rebin:
        ...
        if the_boss.data.q_min is None:
            debug.log(
                "Linearly rebinning data into "
                + str(the_boss.data.n_qvectors)
                + " uniformly spaced "
                + "points in q-space.",
                unimportance=2,
            )
    ...
    elif not the_boss.reduction.normalisation:
        data = np.array(
            [refl.q_vectors, refl.reflectivity_nonorm, refl.reflectivity_e_nonorm]
        ).T
    ...
    # Write the data.
    np.savetxt(filename, data, header=f"{dump(vars(the_boss))}\n Q(1/Å)\tR\tR_error")
